...
async def get_universe():
    """List of all F&O universe symbols with lot sizes."""
    try:
    ...
    return {
        "count":   len(symbols),
        "symbols": [
            {"symbol": s, "lot_size": lot_sizes.get(s, 1)}
            for s in symbols
        ],
    }
...
async def serve_index():
    ...
    return {'status': 'Market Pulse Pro v2 API running'}
...
@app.get('/stock.html')
async def serve_stock_html():
    return FileResponse(os.path.join(frontend_path, 'stock.html'))

# Mount static assets (CSS, JS, images) but NOT at root
