...
def _build_maps(instruments: list[dict]) -> tuple[list[str], dict[str, str], dict[str, int]]:
    ...
    # ------------------------------------------------------------------
    # Pass 2 — NSE EQ: resolve equity tokens for each underlying
    # ------------------------------------------------------------------
    ...
    for inst in instruments:
        if inst.get("exch_seg") != "NSE":
            continue
        if inst.get("instrumenttype") != "EQ":
            continue

        raw_sym = inst.get("symbol", "")
        symbol = raw_sym.replace("-EQ", "").replace("-BE", "").strip()

        if symbol in seen_underlyings:
            token_map[symbol] = str(inst.get("token", ""))

    # ------------------------------------------------------------------
    # Final universe — require BOTH a FUTSTK contract AND an equity token
    # ------------------------------------------------------------------
    final_symbols: list[str] = sorted(s for s in seen_underlyings if s in token_map)

    missing_tokens = sorted(seen_underlyings - set(token_map.keys()))
    if missing_tokens:
        logger.warning(
            "%d F&O underlyings have no NSE EQ token (index/ETF underlyings excluded): %s",
            len(missing_tokens),
            ", ".join(missing_tokens[:20]) + ("…" if len(missing_tokens) > 20 else ""),
        )

    logger.info(
        "Final universe: %d symbols (had token), %d dropped (no EQ token).",
        len(final_symbols),
        len(missing_tokens),
    )
