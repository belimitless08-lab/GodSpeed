...
LOGIN_URL         = f"{SMARTAPI_BASE}/rest/auth/angelbroking/user/v1/loginByPassword"
...
# ---------------------------------------------------------------------------
# Binary tick parser — AngelOne FULL mode (mode=3)
# ---------------------------------------------------------------------------
# FULL mode packet layout (little-endian):
#   Byte  0      : subscription_mode   (uint8)
#   Byte  1      : exchange_type       (uint8)
...
#   Bytes 124-131: last_traded_time    (int64, ms)
#   Bytes 132-139: open_interest       (int64)
#   Bytes 140-147: open_interest_chg   (int64)
#   Bytes 148-155: upper_circuit       (int64, price × 100)
#   Bytes 156-163: lower_circuit       (int64, price × 100)
#   Bytes 164-171: 52w_high            (int64, price × 100)
#   Bytes 172-179: 52w_low             (int64, price × 100)

_FULL_STRUCT = struct.Struct("<BB26sqqqqqddqqqqqqqqqqq")
_FULL_SIZE   = _FULL_STRUCT.size
...
async def get_fresh_session() -> dict:
    """
    Authenticate with AngelOne SmartAPI and return a fresh session dict.
    Never reuse across reconnects.

    Returns
    -------
    {"jwt": str, "feed_token": str, "api_key": str, "client_code": str}

    Raises
    ------
    RuntimeError on any login failure.
    """
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            raise RuntimeError(
                f"[options_ws] Login failed — HTTP {exc.response.status_code}: "
                f"{exc.response.text[:200]}"
            ) from exc
        except httpx.RequestError as exc:
            raise RuntimeError(f"[options_ws] Login failed — network error: {exc}") from exc

    body = resp.json()
    if not body.get("status"):
        raise RuntimeError(
            f"[options_ws] Login rejected by AngelOne: {body.get('message', body)}"
        )
    ...
    if not jwt or not feed_token:
        raise RuntimeError(
            f"[options_ws] Login response missing jwtToken or feedToken: {data}"
        )

    logger.info("[options_ws] Fresh session obtained for client %s.", cfg.ANGELONE_CLIENT_ID)
    return {
        "jwt":         jwt,
...
async def _is_nse_open(jwt: str) -> bool:
    """Return True only when NSE status is explicitly 'OPEN'."""
    headers = {
        "Authorization": f"Bearer {jwt}",
        "X-PrivateKey":  cfg.ANGELONE_API_KEY,
        "X-ClientCode":  cfg.ANGELONE_CLIENT_ID,
        "Content-Type":  "application/json",
        "Accept":        "application/json",
    }
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            resp = await client.post(MARKET_STATUS_URL, json={"exchange": "NSE"}, headers=headers)
            resp.raise_for_status()
        body = resp.json()
    except Exception as exc:
        logger.warning("[options_ws] Market status check failed: %s — treating as CLOSED.", exc)
        return False
    ...
    is_open = isinstance(status, str) and status.upper() == "OPEN"
    logger.info("[options_ws] NSE market status: %s → is_open=%s", status, is_open)
    return is_open
