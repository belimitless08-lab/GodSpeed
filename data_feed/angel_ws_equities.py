...
async def get_fresh_session() -> dict:
    ...
    -------
    {
        "jwt":         str,   # Bearer token for REST + WS auth
        "feed_token":  str,   # WebSocket-specific token
        "api_key":     str,
        "client_code": str,
    }
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            raise RuntimeError(
                f"[angel_ws] Login failed — HTTP {exc.response.status_code}: "
                f"{exc.response.text[:200]}"
            ) from exc
        except httpx.RequestError as exc:
            raise RuntimeError(
                f"[angel_ws] Login failed — network error: {exc}"
            ) from exc

    body = resp.json()
    if not body.get("status"):
        raise RuntimeError(
            f"[angel_ws] Login rejected by AngelOne: {body.get('message', body)}"
        )
    ...
    if not jwt or not feed_token:
        raise RuntimeError(
            f"[angel_ws] Login response missing jwtToken or feedToken: {data}"
        )

    logger.info(
        "[angel_ws] Fresh session obtained for client %s.", cfg.ANGELONE_CLIENT_ID
    )
    return {
        "jwt":         jwt,
...
async def _is_nse_open(jwt: str) -> bool:
    """
    Query AngelOne market-status API.
    Returns True only when NSE status is explicitly "OPEN".
    On any error, returns False so we stay safe and wait.
    """
    headers = {
    ...
    # Payload asks for NSE market status
    payload = {"exchange": "NSE"}

    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            resp = await client.post(MARKET_STATUS_URL, json=payload, headers=headers)
            resp.raise_for_status()
        body = resp.json()
    except Exception as exc:
        logger.warning("[angel_ws] Market status check failed: %s — treating as CLOSED.", exc)
        return False

    # Navigate the response; AngelOne returns a list under data or a single dict
    data = body.get("data", {})
    status = None
    ...
    logger.info("[angel_ws] NSE market status: %s → is_open=%s", status, is_open)
    return is_open
...
_QUOTE_STRUCT = struct.Struct("<BB26sqqqqqddqqqq")
